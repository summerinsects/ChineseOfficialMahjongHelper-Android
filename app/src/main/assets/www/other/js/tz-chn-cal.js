var CHN_CAL = (function () {
    function CHN_CAL() {
    }

    var inst = CHN_CAL;

    inst.MONTH_TEXT = ['\u6B63', '\u4E8C', '\u4E09', '\u56DB', '\u4E94', '\u516D', '\u4E03', '\u516B', '\u4E5D', '\u5341', '\u5341\u4E00', '\u5341\u4E8C'];

    inst.DATE_TEXT = ['\u521D\u4E00', '\u521D\u4E8C', '\u521D\u4E09', '\u521D\u56DB', '\u521D\u4E94', '\u521D\u516D', '\u521D\u4E03', '\u521D\u516B', '\u521D\u4E5D', '\u521D\u5341', '\u5341\u4E00', '\u5341\u4E8C', '\u5341\u4E09', '\u5341\u56DB', '\u5341\u4E94', '\u5341\u516D', '\u5341\u4E03', '\u5341\u516B', '\u5341\u4E5D', '\u4E8C\u5341', '\u5EFF\u4E00', '\u5EFF\u4E8C', '\u5EFF\u4E09', '\u5EFF\u56DB', '\u5EFF\u4E94', '\u5EFF\u516D', '\u5EFF\u4E03', '\u5EFF\u516B', '\u5EFF\u4E5D', '\u4E09\u5341'];

    // 24节气，顺序适配公历月份，小寒放最前面
    inst.SOLAR_TERMS_TEXT = ['\u5C0F\u5BD2', '\u5927\u5BD2', '\u7ACB\u6625', '\u96E8\u6C34', '\u60CA\u86F0', '\u6625\u5206', '\u6E05\u660E', '\u8C37\u96E8', '\u7ACB\u590F', '\u5C0F\u6EE1', '\u8292\u79CD', '\u590F\u81F3', '\u5C0F\u6691', '\u5927\u6691', '\u7ACB\u79CB', '\u5904\u6691', '\u767D\u9732', '\u79CB\u5206', '\u5BD2\u9732', '\u971C\u964D', '\u7ACB\u51AC', '\u5C0F\u96EA', '\u5927\u96EA', '\u51AC\u81F3'];

    inst.greg_leap = function(y) {
        // 75% y % 4 == 0
        // 24% (y % 4 == 0) && (y % 100 != 0)
    ...
    inst._GD_TBL = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

    inst.greg_days = function(y, m) {
        // (91_2/3)% m != 2
        // 6.25% (m != 2) && (y % 4 == 0)
        // 2% (m != 2) && (y % 4 == 0) && (y % 100 != 0)
        // (1/12)% (m != 2) && (y % 4 == 0) && (y % 100 != 0) && (y % 400 == 0)
        return ((m != 2 || !CHN_CAL.greg_leap(y)) ? CHN_CAL._GD_TBL[m - 1] : 29);
    };
    ...
    inst.YEAR_START = 1799;

    // 24节气基础日期
    inst._STBASE = [5, 20, 4, 18, 5, 20, 4, 19, 5, 20, 5, 21, 7, 22, 7, 22, 7, 22, 8, 23, 7, 22, 7, 21];
    inst._STO_18 = [ 0,  0, 77, 57,  0,  0, 48, 88,  0, 88, 44,  0, 96, 64,  0, 96, 36, 80,  0,  0,  0,  0, 84, 48, 0x83a6c8];
    inst._STO_19 = [49,  0,  0, 93, 48, 56, 76,  0, 44,  0, 68,  0,  0, 84, 48,  0, 60,  0,  0,  0,  0,  0,  0, 88, 0x83e7f9];
    inst._STO_20 = [85, 48, 17,  0, 84, 84,  0,  0, 68,  0, 88, 48, 20,  0, 68,  0, 84,  0, 44, 60, 60, 48, 24,  0];
    inst._STO_21 = [ 0, 89, 53,  0,  0,  0,  0, 40, 92,  0,  0, 72, 36,  0, 88, 40,  0,  0, 68, 88, 92, 80, 56,  0, 0x008080];
    inst._STO_22 = [ 0,  0, 89, 65,  0,  0,  0, 68,  0, 52,  0, 88, 56,  0,  0, 60,  0, 56, 96,  0,  0,  0, 92, 64, 0x828288];

    inst._div = function(a, b) {
        var q = 0, b0, n;
        if (a > b) {
            if (b !== 1) {
                b0 = b;
    ...
    inst.solar_term = function(y, i) {
        var v = CHN_CAL._STI[y - CHN_CAL.YEAR_START],
        s = CHN_CAL._STBASE[i] + ((v >> i) & 1),
        d = y % 100,
        c = (y - d) / 100,
        o = (c === 20) ? CHN_CAL._STO_20 : (c === 19 ? CHN_CAL._STO_19 : (c === 21 ? CHN_CAL._STO_21 : (c >= 22 ? CHN_CAL._STO_22 : (c <= 18 ? CHN_CAL._STO_18 : 0)))),
        t = o[24];
        if (t && ((t >> i) & 1)) ++s;
        t = o[i];
        if (t && y >= t + 100 * c) --s;
    ...
    inst.solar_terms = function(y, m) {
        var i = ((m - 1) << 1);
        return [CHN_CAL.solar_term(y, i), CHN_CAL.solar_term(y, i + 1)];
    };

    inst._ofst = function(y, m, d) {
        if (m < 3) {
            m += 12;
            --y;
        }
        return y * 365 + (y >> 2) - CHN_CAL._div(y, 100)['q'] + CHN_CAL._div(y, 400)['q'] + (m - 3) * 30 + (((m - 2) * 19) >> 5) + d;
    };

    inst._new_moons = function(y, nm, o) {
        var w,
        b,
        v,
        s,
        ...
        w = CHN_CAL._ofst(y, 12, CHN_CAL.solar_term(y, 23));  // 当年的冬至

        // 当年朔日信息
        b = CHN_CAL._NMI[y - CHN_CAL.YEAR_START];

        // 首个朔日
        v = (b >> 12) & 0xfff;
        s = CHN_CAL._ofst(y, 1, v);
        nm[o] = s;
        ...
        // 越界检查
        if (s + v - nm[o] > (CHN_CAL.greg_leap(y) ? 366 : 365)) {
            nm[o + i] = 0;
        }
    ...
    inst.greg2chn = function(o) {
        ...
        // 待计算日期在冬至所在月或之后
            ...
            // 再计算下一年信息
            y1 = y + 1;
            w1 = CHN_CAL._new_moons(y1, nm, nm[12] ? 13 : 12);
        }
        ...
        if (w1 - w > 12) {
            // 中气
            for (i = 0; i < 11; ++i) {
                mt[i] = CHN_CAL._ofst(y1, i + 1, CHN_CAL.solar_term(y1, i * 2 + 1));
            }
        ...
        return {'y':y,'m':m,'d':s - nm[k - 1] + 1,'j':nm[k] - nm[k - 1] === 30,'l':!!l && l === k};
    };

    inst.chn_date_text = function(o) {
        var s = '';
        if (o['l']) s += '\u95F0';
        s += CHN_CAL.MONTH_TEXT[o['m'] - 1];
        s += '\u6708';
        s += CHN_CAL.DATE_TEXT[o['d'] - 1];
        return s;
    };

    // 日干支
    inst.greg_hseb = function(y, m, d) {
        /**
        ...
        return (y * 5 + (y >> 2) - CHN_CAL._div(y, 100)['q'] + CHN_CAL._div(y, 400)['q'] + ((m - 1) & 1) * 30 + (((m - 2) * 19) >> 5) + d + 8) % 60;
    };
