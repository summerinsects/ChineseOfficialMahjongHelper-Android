...
public final class CalendarUtils {
    public static final class GregorianDate {
        ...
        public void assign(final GregorianDate date) {
            year = date.year;
            month = date.month;
            day = date.day;
        }
    ...
    public static final class ChineseDate {
        public int year;
        public int month;
        public int day;
        ...
        public void assign(final ChineseDate date) {
            year = date.year;
            month = date.month;
            day = date.day;
            major = date.major;
    ...
    public static final int CALENDAR_MIN_YEAR = 1800;
    public static final int CALENDAR_MAX_YEAR = 2299;

    public static boolean Gregorian_IsLeapYear(int y) {
        // 75% y % 4 == 0
        // 24% (y % 4 == 0) && (y % 100 != 0)
    ...
    private static final int[] DAYS_TABLE = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

    public static int Gregorian_DaysInMonth(int y, int m) {
        // (91_2/3)% m != 2
        // 6.25% (m != 2) && (y % 4 == 0)
        // 2% (m != 2) && (y % 4 == 0) && (y % 100 != 0)
        // (1/12)% (m != 2) && (y % 4 == 0) && (y % 100 != 0) && (y % 400 == 0)
        return ((m != 2 || !Gregorian_IsLeapYear(y)) ? DAYS_TABLE[m - 1] : 29);
    }

    public static int Gregorian_WeekDay(int y, int m, int d) {
        // 基姆拉尔森计算公式优化
        if (m == 1 || m == 2) {
            m += 12;
            --y;
    ...
    private static final int _CALENDAR_YEAR_START = CALENDAR_MIN_YEAR - 1;
     ...
     */
    private static final int[] _NEW_MOONS = {
            0x062ad,
     ...
     */
    private static final int[] _SOLAR_TERMS = {
            0x000000,
    ...
    // 24节气基础日期
    private static final int[] _SOLAR_TERMS_BASE = {
            5, 20, 4, 18, 5, 20, 4, 19, 5, 20, 5, 21, 7, 22, 7, 22, 7, 22, 8, 23, 7, 22, 7, 21
    };

    private static final int _SOLAR_TERMS_BASE_OFFSET_18XX = 0x83a6c8;
    private static final int _SOLAR_TERMS_BASE_OFFSET_19XX = 0x83e7f9;
    private static final int _SOLAR_TERMS_BASE_OFFSET_21XX = 0x008080;
    private static final int _SOLAR_TERMS_BASE_OFFSET_22XX = 0x828288;
    private static final int[] _SOLAR_TERMS_OFFSET_TABLE_18XX = {
            0,  0, 77, 57,  0,  0, 48, 88,  0, 88, 44,  0, 96, 64,  0, 96, 36, 80,  0,  0,  0,  0, 84, 48
    };
    private static final int[] _SOLAR_TERMS_OFFSET_TABLE_19XX = {
            49,  0,  0, 93, 48, 56, 76,  0, 44,  0, 68,  0,  0, 84, 48,  0, 60,  0,  0,  0,  0,  0,  0, 88
    };
    private static final int[] _SOLAR_TERMS_OFFSET_TABLE_20XX = {
            85, 48, 17,  0, 84, 84,  0,  0, 68,  0, 88, 48, 20,  0, 68,  0, 84,  0, 44, 60, 60, 48, 24,  0
    };
    private static final int[] _SOLAR_TERMS_OFFSET_TABLE_21XX = {
            0, 89, 53,  0,  0,  0,  0, 40, 92,  0,  0, 72, 36,  0, 88, 40,  0,  0, 68, 88, 92, 80, 56,  0
    };
    private static final int[] _SOLAR_TERMS_OFFSET_TABLE_22XX = {
            0,  0, 89, 65,  0,  0,  0, 68,  0, 52,  0, 88, 56,  0,  0, 60,  0, 56, 96,  0,  0,  0, 92, 64
    };

    private static int _DayOffset(int y, int m, int d) {
        if (m < 3) {
            m += 12;
            --y;
        }
    ...
    private static int _GetNewMoons(int y, int[] new_moons, int start_idx) {
        int sum, nm1;
        int i0, i;

        final int w = _DayOffset(y, 12, Gregorian_GetSolarTerm(y, 23));  // 当年的冬至

        // 当年朔日信息
        final int bit = _NEW_MOONS[y - _CALENDAR_YEAR_START];
        ...
        // 越界检查
        if (sum + nm1 - new_moons[start_idx] > (Gregorian_IsLeapYear(y) ? 366 : 365)) {
    ...
    public static ChineseDate Gregorian2Chinese(final GregorianDate date) {
        int y = date.year;
        int m = date.month;
        int d = date.day;
        ChineseDate res = new ChineseDate();
        if (y < _CALENDAR_YEAR_START || y > CALENDAR_MAX_YEAR || m < 1 || m > 12 || d < 1 || d > 31) {
            return res;
        }

        int[] mid_terms = new int[11];
        int[] new_moons = new int[26];
        int o;
        int y1;
        int w, w1, leap, i, k;
        ...
        // 待计算日期在冬至所在月或之后
            ...
            // 再计算下一年信息
            y1 = y + 1;
            w1 = _GetNewMoons(y1, new_moons, new_moons[12] != 0 ? 13 : 12);
        }
    ...
    public static int Gregorian_GetSolarTerm(int y, int i) {
        final int v = _SOLAR_TERMS[y - _CALENDAR_YEAR_START];
        int s = _SOLAR_TERMS_BASE[i] + ((v >> i) & 1);
        int c = y / 100;
        int t = 0;
        if (c == 20) {
            t = _SOLAR_TERMS_OFFSET_TABLE_20XX[i];
        } else if (c == 19) {
            if (((_SOLAR_TERMS_BASE_OFFSET_19XX >> i) & 1) != 0) ++s;
            t = _SOLAR_TERMS_OFFSET_TABLE_19XX[i];
        } else if (c == 21) {
            if (((_SOLAR_TERMS_BASE_OFFSET_21XX >> i) & 1) != 0) ++s;
            t = _SOLAR_TERMS_OFFSET_TABLE_21XX[i];
        } else if (c >= 22) {
            if (((_SOLAR_TERMS_BASE_OFFSET_22XX >> i) & 1) != 0) ++s;
            t = _SOLAR_TERMS_OFFSET_TABLE_22XX[i];
        } else /*if (c <= 18)*/ {
            if (((_SOLAR_TERMS_BASE_OFFSET_18XX >> i) & 1) != 0) ++s;
    ...
    public static int Gregorian_GetStemBranch(int y, int m, int d) {
        /**
